...
// Given an 'index' , return the text that goes with this radio button
...
function getTopicLength( )
{
       ...
       return  myGlobals.TREE_TOPICS.length;
...
// EventHandler:  called when a radio button is selected
...
//	REPLACED BY reading the topic questions from XML file

	// Get the pnode location in the html file
	pnode=document.getElementById('topic');
	
        len = getTopicLength( ) ;                               // one radio button for each topic
	for(i=0 ; i < len ; i++){
	
	 // Add a radio button
         theNode = addNode(pnode, 'input', 'topicName','23');
	 theNode.type = 'radio';
	 theNode.onclick = constructQuestionHandler;   			    // just for reference 
//	 theNode.setAttribute("onclick" , "constructQuestionHandler();");   // these are equivalent

//	 if(i==0) theNode.checked = "true";	 		// no longer have a default one selected
	 
	 // Add the text and a line break
	 addNode(pnode,"text","Country", getTopic(i) );       // topics already read from an XML file
